...
CONCEPT_KEYWORDS: list[tuple[str, tuple[str, ...]]] = [
    ...
    # --- 전수조사로 추가 (오너 확정) ---
    ('#대화중심', ('대화', '토크', '대화형', '대화중심')),   # ※ '티키타카'는 감정오렌지 보편어라 제외
    ('#프리미엄', ('프리미엄', '능력특집', '능력자', '고소득', '하이엔드', 'vip')),
    ('#결혼전제', ('결혼전제', '결혼특집', '결혼', '진지한만남')),
]

# 형식 (중간)
FORMAT_KEYWORDS: list[tuple[str, tuple[str, ...]]] = [
    ('#1:1', ('1:1', '1대1', '일대일', '원온원', '1 대 1')),
    ('#2:2', ('2:2', '2대2', '이대이', '2 대 2')),
    ('#소규모', ('소규모', '소수정예', '스몰', '소인원', '소수')),
    # '커피'·'카페'까지 넓힘 (오너 확정) — 대부분의 커피미팅형 소개팅 포착
    ('#커피미팅', ('커피', '카페', '커피미팅', '커피 미팅', '커피챗', '티타임',
                '카페모임', '커피타임')),
    ('#식사모임', ('식사모임', '디너', '만찬', '저녁식사', '점심모임', '식사',
                '다이닝', '맛집')),
    ('#사회자진행', ('사회자', '진행자', '호스트진행', '사회자진행', '진행자가')),
    # ❌ #로테이션 제거 — 거의 전 이벤트에 해당하는 보편어라 특색 아님 (오너 확정)
]

# 대상 (키워드 기반; 연령대는 별도 나이 범위로 산출)
TARGET_KEYWORDS: list[tuple[str, tuple[str, ...]]] = [
    ('#직장인', ('직장인', '회사원', '샐러리맨', '워커', '직장인만')),
    ('#전문직', ('전문직', '의사', '변호사', '약사', '회계사', '변리사',
               '판사', '검사', '교수', '세무사', '감정평가사', '전문직만')),
    # 공무원·교사는 전문직과 별개로 각각 구분 (오너 확정)
    ('#대기업', ('대기업', '대기업재직', '대기업만')),
    ('#공무원', ('공무원', '공기업')),
    ('#교사', ('교사', '교직', '선생님')),
]
