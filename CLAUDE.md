> ## ⚠️ 먼저 읽을 것 (2026-09-07)
> **이 문서보다 저장소 루트의 `AGENTS.md` 를 먼저 읽으세요.**
> 이 문서는 초기 기획 단계에 쓴 것이라 지금과 다른 대목이 있습니다.
>
> **지금은 사실이 아닌 것:**
> - "업체 직접 등록 기능 구현 금지" → **지금은 제휴 포털(`partner/`)에서 업체가 직접 등록합니다**
> - "MVP Scope 외 기능 금지" → 소셜링·혼술바·커뮤니티·제휴 포털까지 이미 들어갔습니다
> - 아래 «서브 에이전트» 체계는 지금 쓰지 않습니다(참고용)
>
> **여전히 맞는 것:** 자체 결제·예약 금지(전부 아웃링크), 앱 회원가입 없음, 키 하드코딩 금지
>
> 현재 상태·규칙·크롤링은 `AGENTS.md` → `docs/OWNER_RULES.md` → `docs/CRAWLING.md` 순으로 보세요.
...
4. **API 키를 코드에 하드코딩 금지** — 반드시 환경변수 사용
5. **PRD.md의 MVP Scope 외 기능 임의 구현 금지**

<!-- 2026-08-11 오너 지시로 "로그인이 필요한 페이지 크롤링 금지" 원칙 제거.
     인스타그램 등이 익명 방문자에게 bio_links/external_url을 안 내려주는 사례가
     늘어 필요 시 로그인 세션으로 크롤링 가능. 계정 확보·자격증명 관리는 건별로 논의. -->
