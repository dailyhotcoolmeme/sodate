...
### 반드시 버튼을 눌러서 정보를 수집한다
모든 이벤트 정보(나이대, 참가자 현황, 잔여석 등)는 **사이트 최초 진입 시 보이는 텍스트만으로 파싱하는 것을 금지**한다.
반드시 Playwright를 사용하여 각 이벤트의 신청/상세 버튼을 클릭하고, 클릭 후 나타나는 모달·팝업·상세 페이지에서 정확한 정보를 수집해야 한다.

```
원칙:
  1. 목록 페이지 → 각 이벤트 카드/버튼 클릭
  2. 클릭 후 열리는 상세 팝업·모달·페이지 파싱
  3. 나이대, 참가자 현황, 잔여석은 반드시 이 방식으로 수집
  4. 버튼 클릭 없이 얻을 수 없는 정보는 null로 저장 금지 — 직접 클릭해서 확인
```

> 이 원칙을 지키지 않으면 잘못된 데이터(빈값·불완전한 나이대·현황 누락)가 DB에 저장되어
> 앱에서 현황 버튼과 나이대 배지가 표시되지 않는다.

---

## ⛔ 절대 금지
<!-- 2026-08-11 오너 지시로 "로그인 필요 페이지 크롤링 금지" 제거. 인스타 등이 익명
     방문자에게 bio_links/external_url을 안 내려주는 사례가 늘어 필요 시 로그인
     세션으로 크롤링 가능(계정·자격증명 관리는 건별 논의). -->
- 1초 이하 간격으로 연속 요청 금지 (요청 간 최소 1~2초 딜레이)
- 개인정보(이름, 연락처 등) 크롤링 절대 금지
- SUPABASE_SERVICE_ROLE_KEY 코드에 하드코딩 금지
- 사이트 진입 시 보이는 텍스트만으로 나이대·참가자 현황 파싱 금지 (반드시 버튼 클릭 후 수집)
