...
export default function TopBar({
  ...
  const styles = useMemo(() => StyleSheet.create({
    ...
    // 하트+"모잇"이 한 이미지로 된 워드마크(690x231 = 2.99:1).
    ...
    // 소개팅|커뮤니티 알약(글자 14)과 덩치를 맞춘다. 690x231 = 2.99:1 이라
    // 높이 24 에 맞는 폭은 72 다.
    // ⚠️ 글자 높이는 하트의 90%(208/231). 두 글자뿐이라 그보다 작으면 하트에 눌린다.
    // ⚠️ '모잇' 글자는 배달의민족 주아(BM Jua)로 조판한다. 옛 워드마크 글자를 잘라
    //    붙이는 방식은 균형이 안 맞아 폐기했다(2026-08-31). 하트만 원본 그대로다.
    //    크기·자간을 바꾸려면 assets/build-logos.py 를 돌리고 여기 상수도 갱신.
    logoWordmark: { width: 72, height: 24 },
