...
export default function TopBar({
  ...
  const styles = useMemo(() => StyleSheet.create({
    ...
    // 하트+"모잇"이 한 이미지로 된 워드마크(667x231 = 2.89:1).
    ...
    // 소개팅|커뮤니티 알약(글자 14)과 덩치를 맞춘다. 667x231 = 2.89:1 이라
    // 높이 24 에 맞는 폭은 69 다.
    // ⚠️ 글자 높이는 하트의 90%(208/231). 두 글자뿐이라 그보다 작으면 하트에 눌린다.
    // ⚠️ '모잇' 글자는 옛 '소개팅모아' 워드마크의 글자를 잘라 붙여 만들었다
    //    (모 = 통째 복사 / 잇 = 팅의 ㅇ·ㅣ + 소의 ㅅ). 만드는 규칙은
    //    assets/build-logos.py 에 있다 — 크기·비율을 바꾸려면 그걸 돌린다.
    logoWordmark: { width: 69, height: 24 },
