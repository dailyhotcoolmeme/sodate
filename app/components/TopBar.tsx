...
export default function TopBar({
  ...
  const styles = useMemo(() => StyleSheet.create({
    ...
    // 하트+"소밋"이 한 이미지로 된 워드마크(원본 592x231 = 2.56:1).
    // ⚠️ 2026-08-31 앱 이름을 '소개팅모아' → '소밋'으로 바꾸며 글자만 다시 그렸다.
    //    하트는 원본 그대로 두고, 글자는 원래 워드마크에서 실측한 뼈대(세로획 31,
    //    가로획 24, 음절 간격 150, ㅁ 왼쪽아래·ㅅ 오른쪽위만 둥근 모서리)로 같은
    //    글씨체를 재현했다. 다시 그릴 일이 있으면 assets/logo-wordmark-source.py 참고.
    // 라이트·다크 양쪽에서 보이는 핑크 버전만 사용(검정/흰색은 한쪽에서 사라짐).
    // 소개팅|커뮤니티 알약(글자 14)과 덩치를 맞춘다. 592x231 = 2.56:1 이라
    // 높이 24 에 맞는 폭은 62 다(글자 수가 줄어 108 → 62).
    logoWordmark: { width: 62, height: 24 },
